...
import { supabaseAdmin } from '@/utils/supabase/client/supabaseAdmin'; // <-- Import the service client directly here
...
export const fetchDashboardStats = async (): Promise<[any | null, any | null]> => {
  try {
    ...
    // Get total bookings
    const { count: totalBookings, error: totalBookingsError } = await supabaseAdmin // <-- Use supabaseAdmin directly
      .from('tbl_bookings')
      .select('*', { count: 'exact', head: true });

    // Get total revenue (completed bookings only)
    const { data: revenueData, error: revenueError } = await supabaseAdmin // <-- Use supabaseAdmin directly
      .from('tbl_bookings')
      .select('total_amount')
      .eq('status', 'completed');

    // Get total customers
    const { count: totalCustomers, error: totalCustomersError } = await supabaseAdmin // <-- Use supabaseAdmin directly
      .from('tbl_customers')
      .select('*', { count: 'exact', head: true });

    // Get today's bookings
    const { count: todayBookings, error: todayBookingsError } = await supabaseAdmin // <-- Use supabaseAdmin directly
      .from('tbl_bookings')
      .select('*', { count: 'exact', head: true })
...
// These files will now directly import `supabaseAdmin` from its source.
